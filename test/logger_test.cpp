...
TEST(LogDataTest, LogDataInfoTest) {
	...
	std::string without_time = data.get_message().substr(10);

	EXPECT_EQ(without_time, "[INFO] Some info: 42, and another one\n");
};
...
TEST(LogDataTest, LogDataErrorTest) {
	...
	EXPECT_EQ(without_time, "[ERROR] Some error: 42, and another one\n");
};
...
TEST(LogDataTest, LogDataDebugTest) {
	...
	EXPECT_EQ(without_time, "[DEBUG] [test/logger_test.cpp:26] Some debug: 42, and another one\n");
};
...
TEST(LoggerTest, LogInfoTest) {
	...
	EXPECT_EQ(without_time, "\x1B[32m[INFO] Some info: 42, and another one\n\x1B[0m");
};
...
TEST(LoggerTest, LogErrorTest) {
	...
	EXPECT_EQ(without_time, "\x1B[31m[ERROR] Some error: 42, and another one\n\x1B[0m");
};
...
TEST(LoggerTest, LogDebugTest) {
	...
	EXPECT_EQ(without_time, "\x1B[33m[DEBUG] [test/logger_test.cpp:64] Some debug: 42, and another one\n\x1B[0m");
};
